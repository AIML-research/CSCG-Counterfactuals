...
def parse_args():
    ...
    parser.add_argument(
        "--target-data", default="data.npy", dest="data_filename", type=str
    )
...
# ! New one
def pick_instance(model, dataset_name, idx=None):
    dir_path = os.path.dirname(os.path.realpath(__file__))
    file_path = f"{dir_path}/processed_{dataset_name}_n=100.csv"
    experiment_instances = np.loadtxt(file_path, delimiter=",")
    target_instance = experiment_instances[i, :]
    target_label = 1.0 - model.predict(np.array(target_instance).reshape(1, -1))[0]
    return target_instance, target_label, idx
...
def run_on_instance(options, instance_id, sav_dir=None):
    ...
    with tf.Session() as session:
        env = load_env(
            options.model_name, options.data_filename, used_actions=options.action_names
        )
        model = load_model(options.model_name, options.model_ckpt)
        instance, label, instance_id = pick_instance(
            model, options.model_name, idx=instance_id
        )
        ...
        if options.model_name == "quickdraw":
            actions = [
                action.set_p_selector(i, len(actions))
                for i, action in enumerate(actions)
            ]

        heuristics = load_heuristics(options.mode, actions, model, options.length)
        search = SequenceSearch(
            model, actions, heuristics, sav_dir=sav_dir, config=base_config
        )

        if options.model_name == "quickdraw":
            result = search.find_correction(
        ...
        out = dict(
            info=get_instance_info(instance, features),
...
def run_on_instance(options, instance_id, sav_dir=None):
    ...
    with tf.Session() as session:
        ...
        # print(get_instance_info(instance, features))
        out = dict(
            info=get_instance_info(instance, features),
            output=result.summary() if result.best_result is not None else "Not Found",
        )
...
def start_recording(target_idx, options):
    ...
    print(
        "Starting %s run on item %d at %d" % (options.mode, target_idx, info["start"])
    )
    return info
...
def end_recording(record, result):
    ...
    print(
        "Finished %s run on item %d at %d, time taken: %d"
...
if __name__ == "__main__":
    ...
    folder_name = f"competitor_{now.strftime('%d-%m-%Y')}_{FLAGS.model_name}"
    Path(f"{dir_path}/results/{folder_name}/").mkdir(parents=True, exist_ok=True)

    config_path = f"{dir_path}/results/{folder_name}/config_{now.strftime('%d-%m-%Y')}_{FLAGS.model_name}.json"

    with open(config_path, "w") as f:
        json.dump(vars(base_config), f, indent=4)
    ...
    for i in range(100):
        ...
        if FLAGS.model_name != "quickdraw" or output["success"]:
            file_path = f"{dir_path}/results/{folder_name}/run_{i}_{now.strftime('%d-%m-%Y')}_{FLAGS.model_name}.json"
