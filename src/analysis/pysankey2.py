...
'''
Code if from https://github.com/SZJShuffle/pySankey2 with a few adjustments.
'''
...
class Sankey:
    ...
    def __init__(
        ...
        self.dataFrame.columns = [
            "layer%d" % (i + 1) for i in range(dataFrame.shape[1])
        ]
        ...
        if layerLabels is None:
            ...
            self._checkLayerLabelsMatchDF(
                self.dataFrame, layerLabels, self._colnameMaps
            )
        ...
        if colorMode not in _opts:
            raise ValueError(
                "colorMode options must be one of:{0} ".format(
                    ",".join([i for i in _opts])
                )
            )
    ...
    def _getLayerLabels(self, dataFrame):
        ...
        for layer_label in dataFrame.columns:
            layer_labels = list(
                dataFrame.loc[:, layer_label].unique()
            )  # may contain NaN
            ...
            for lab in layer_labels:
                counts.append(
                    (
                        (dataFrame[dataFrame.loc[:, layer_label] == lab])
                        .loc[:, layer_label]
                        .count()
                    )
                )
            # ! resort for count
            layer_labels = np.array(layer_labels)[np.argsort(counts)[::-1]].tolist()
    ...
    def _checkLayerLabelsMatchDF(self, dataFrame, layerLabels, colnameMaps):
        ...
        for oldname, newname in colnameMaps.items():
            ...
            if df_set != provided_set:
                ...
                msg_provided = (
                    "Provided Labels:" + ",".join([str(i) for i in provided_set]) + "\n"
                )
                raise LabelMismatchError(
                    "{0} do not match with {1}".format(msg_provided, msg_df)
                )
    ...
    def _checkColorMatchLabels(self, colorDict, mode):
        ...
        if mode == "global":
            ...
            if provided_set != df_set:
                msg_provided = (
                    "Provided Color Labels:"
                    + ",".join([str(i) for i in provided_set])
                    + "\n"
                )
                msg_df = "dataFrame Labels:" + ",".join([str(i) for i in df_set]) + "\n"
                raise LabelMismatchError(
            ...
            for old_layer, layer_labels_map in colorDict.items():
                ...
                if provided_set != df_set:
                    ...
                    msg_df = (
                        "dataFrame Labels:" + ",".join([str(i) for i in df_set]) + "\n"
                    )
                    raise LabelMismatchError(
                        "In {0},{1} do not match with {1}".format(
                            new_layer, msg_provided, msg_df
                        )
                    )
    ...
    def _setboxPos(self, dataFrame, layerLabels, boxInterv):
        ...
        for layer, labels in layerLabels.items():
            ...
            for i, label in enumerate(labels):
                labelHeight = (
                    (dataFrame[dataFrame.loc[:, layer] == label]).loc[:, layer].count()
                )
                if i == 0:
                    ...
                    layerPos[label]["bottom"] = (
                        prevLabelTop + boxInterv * dataFrame.loc[:, layer].count()
                    )
                    layerPos[label]["top"] = layerPos[label]["bottom"] + labelHeight
                # layerPos[label]["top"] += 1
        ...
        for i, layer in enumerate(layers):
            ...
            for leftLabel in layerLabels[leftLayer]:
                for rightLabel in layerLabels[rightLayer]:
                    width = len(
                        dataFrame[
                            (dataFrame.loc[:, leftLayer] == leftLabel)
                            & (dataFrame.loc[:, rightLayer] == rightLabel)
                        ]
                    )
                    if width > 0:
    ...
    def _setStripPos(
        self, leftBottom, rightBottom, leftTop, rightTop, kernelSize, stripShrink
    ):
        """
        ...
        ys_bottom = np.convolve(
            ys_bottom + stripShrink,
            (1 / kernelSize) * np.ones(kernelSize),
            mode="valid",
        )
        ys_bottom = np.convolve(
            ys_bottom + stripShrink,
        ...
        ys_top = np.convolve(
            ys_top - stripShrink, (1 / kernelSize) * np.ones(kernelSize), mode="valid"
        )
        ys_top = np.convolve(
            ys_top - stripShrink, (1 / kernelSize) * np.ones(kernelSize), mode="valid"
        )
    ...
    def _plotBox(
        ...
        i = 0
        j = 1
        all_labels = list(set([x for a in layerLabels.values() for x in a]))
        for layer, labels in layerLabels.items():
            for label in labels:
                ...
                ax.fill_between(
                    ...
                    y2=labelTop,
                    facecolor=color,
                    hatch=self.hatches[label],
                    alpha=0.9,
                    label=label if label in all_labels else None,
                    **box_kws,
                )
                try:
                ...
                # ! Plot labels only if first layer
                if i == -1:
                    ax.text(
            ...
            # * Plot action seq. number below
            ax.text(
                (layerPos[layer]["layerStart"] + layerPos[layer]["layerEnd"]) / 2,
                (distToBoxBottom - bot_dist),
                # f"Action {j}",
                f"$t={j}$",
                {"ha": "center", "va": "center"},
                fontsize=fontSize,
            ...
            for leftLabel in layerLabels[leftLayer]:
                for rightLabel in layerLabels[rightLayer]:
                    ...
                    if width > 0:
                        ...
                        rightTop = (
                            rightBottom + stripWidths[layer][leftLabel][rightLabel]
                        )

                        ys_bottom, ys_top = self._setStripPos(
                            leftBottom,
                            rightBottom,
                            leftTop,
                            rightTop,
                            kernelSize=kernelSize,
                            stripShrink=stripShrink,
                        )
                        ...
                        if stripColor == "left":
                            if self.colorMode == "global":
                                ax.fill_between(
                                    np.linspace(x_start, x_end, len(ys_top)),
                                    ys_bottom,
                                    ys_top,
                                    alpha=0.8,
    ...
    def plot(
        ...
        """
        # set box position
        self._boxPos = self._setboxPos(
            self.dataFrame, self._layerLabels, boxInterv=boxInterv
        )
        # set layer position
        self._layerPos = self._setLayerPos(
            self._layerLabels, boxWidth=boxWidth, stripLen=stripLen
        )
        # set strip width
