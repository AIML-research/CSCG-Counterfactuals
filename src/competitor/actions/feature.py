...
class BaseFeature(object):
    def __init__(self, name, idx, orig_idx=None):
        self.name = name
        self.idx = idx
...
class Feature(BaseFeature):
    def __init__(self, name, idx, mean, std, input_dim, orig_idx):
        super().__init__(name=name, idx=idx, orig_idx=orig_idx)
        self.mean = mean
        self.std = std
        self.selector_np = np.zeros(input_dim)
        self.selector_np[idx] = 1.0
...
class CategoricFeature(BaseFeature):
    def __init__(self, name, idx, values, input_dim, orig_idx, value_map=None):
        super().__init__(name, idx, orig_idx)
        self.values = values
        self.values_dict = {v: i for i, v in enumerate(values)}
        self.selector = []
    ...
    def get_feature_value(self, instance, use_tensor, space="z"):
        if use_tensor:
            ...
            # ! Mine is more performant, tested by:
            # ! assert (instance[self.idx : self.idx + len(self.values)] == np.array(feature_vector)).all()
            feature_vector = instance[self.idx : self.idx + len(self.values)]
            return np.argmax(feature_vector)

    def change_feature_value(self, instance, value_idx, use_tensor):
        if use_tensor:
            value_remover = -1 * (self.one_hot_selector - 1.0)
            ...
            for i in range(len(self.values)):
                new_instance[self.idx + i] = 1.0 if i == value_idx else 0.0
            return new_instance
...
class StrokeFeature(BaseFeature):
    def __init__(self, name, idx, input_dim):
        ...
        self.selector_np[idx][:] = [1.0, 1.0, 1.0, 1.0]
        self.selector_tf = None
    ...
    def initialize_tf_variables(self):
        self.selector_tf = tf.constant(self.selector_np, dtype=tf.float32)
        self.helper_tf = tf.constant(
            [1, 0, 0, 1, -1, 0, 0, -1], shape=[4, 2], dtype=tf.float32
        )
...
def loader(feature_file):
    ...
    for feature in raw_features:
        if feature["type"] == "numeric":
            ...
            features[feature["name"]] = StrokeFeature(
                feature["name"], feature["idx"], input_dim
            )
        elif feature["type"] != "class":
            features[feature["name"]] = CategoricFeature(
                feature["name"],
                feature["idx"],
                feature["values"],
                input_dim,
